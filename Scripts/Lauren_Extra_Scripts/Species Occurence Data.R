...
library(sp); library(rvertnet); library(rnaturalearth); library(graphics)
...
studyExtent2 <- extent(recordsSpatial)
# Save (you'll be happy about this when your R crashes)
save(studyExtent2, file = './Test/StudyExtent.Rdata', compress = T)
...
sp::plot((ne_countries()), add = T, col = 'gray80')
sp::plot((ne_states()), add = T, col = 'gray80')
...
saveRDS(records, '.Test/MatchingEnvironments.rds')
