######################################################
## MAKING OCC POINTS FOR MICROHABITAT using 7M and 7L ###
######################################################
...
Arb <- Class1[Class1$SemiAqStrict == "A", ]
...
writeOGR(ArbPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/ArbPoly_7M", layer= "chull", driver = "ESRI Shapefile")
#ArbPolyAll <- readOGR("./Analysis_Scripts/Chapter3/Polygons/ArbPoly_7M/chull.shp")
#
### prune terrestrial polys - T ###
Terr <- Class1[Class1$SemiAqStrict == "T", ]
...
writeOGR(TerrPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/TerrPoly_7M", layer= "chull", driver = "ESRI Shapefile")
#TerrPolyAll <-readOGR("./Analysis_Scripts/Chapter3/Polygons/TerrPoly_strict/chull.shp")
#
### prune aquatic polys - W##
Aqua <- Class1[Class1$SemiAqStrict == "W", ]
Aqua$Species # 32
...
writeOGR(AquaPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/AquaPoly_7M", layer= "chull", driver = "ESRI Shapefile")
#AquaPolyAll <-readOGR("./Analysis_Scripts/Chapter3/Polygons/AquaPoly_strict/chull.shp")
#
### prune cave polys - C##
Cave <- Class1[Class1$SemiAqStrict == "C", ]
Cave$Species # 11
...
writeOGR(CavePolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/CavePoly_7M", layer= "chull", driver = "ESRI Shapefile")
#CavePolyAll <- readOGR("./Analysis_Scripts/Chapter3/Polygons/CavePoly_strict/chull.shp")
#
### prune saxicolous polys - S##
Sax <- Class1[Class1$SemiAqStrict == "S", ]
Sax$Species # 7
...
writeOGR(SaxPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/SaxPoly_7M", layer= "chull", driver = "ESRI Shapefile")
#SaxPolyAll <- readOGR("./Analysis_Scripts/Chapter3/Polygons/SaxPoly_strict/chull.shp")
#
### prune fossorial polys - F##
Foss <- Class1[Class1$SemiAqStrict == "F", ]
Foss$Species  # 3
...
writeOGR(FossPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/FossPoly_7M", layer= "chull", driver = "ESRI Shapefile")
...
ArbPoints7M <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                     proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(ArbPoints7M, "./Analysis_Scripts/Chapter3/Points/Arb_Points_7M", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
#
...
maybe <- data.frame(ekek)
maybe
AquaPoints7M <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                      proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(AquaPoints7M, "./Analysis_Scripts/Chapter3/Points/Aqua_Points_7M", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
...
coords <- ekek@coords
maybe <- data.frame(ekek)
CavePoints7M <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                      proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(CavePoints7M, "./Analysis_Scripts/Chapter3/Points/Cave_Points_7M", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
...
maybe <- data.frame(ekek)
maybe
SaxPoints7M <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                     proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(SaxPoints7M, "./Analysis_Scripts/Chapter3/Points/Sax_Points_7M", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
#
...
maybe <- data.frame(ekek)
maybe
FossPoints7M <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                      proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(FossPoints7M, "./Analysis_Scripts/Chapter3/Points/Foss_Points_7M", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
...
#
### prune arboreal polys - A ##
Arb <- Class1[Class1$SemiAqLenient == "A", ]
Arb$Species #56 species
ArbPoly <- Polygons[match(Arb$Species,Polygons$binomial), ]  
ArbPolyAll <- aggregate(ArbPoly, dissolve=T)
# below is making a file for the ArbPolygons but cannot use in making the points because of class ID
ArbPolyAll.SP <- SpatialPolygonsDataFrame(ArbPolyAll, data=data.frame(binomial=1), match.ID = F)
writeOGR(ArbPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/ArbPoly_7L", layer= "chull", driver = "ESRI Shapefile")
#ArbPolyAll <- readOGR("./Analysis_Scripts/Chapter3/Polygons/ArbPoly_strict/chull.shp")
#
### prune terrestrial polys - T ###
Terr <- Class1[Class1$SemiAqLenient == "T", ]
Terr$Species #201 species
TerrPolygon <- Polygons[match(Terr$Species, Polygons$binomial), ] 
TerrPolybuffer <- rgeos::gBuffer(TerrPolygon, width = 0, byid=F)
TerrPolyAll <- aggregate(TerrPolybuffer, dissolve=T)
# below is making a file for the ArbPolygons but cannot use in making the points because of class ID
TerrPolyAll.SP <- SpatialPolygonsDataFrame(TerrPolyAll, data=data.frame(binomial=1), match.ID = F)
writeOGR(TerrPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/TerrPoly_7L", layer= "chull", driver = "ESRI Shapefile")
#TerrPolyAll <-readOGR("./Analysis_Scripts/Chapter3/Polygons/TerrPoly_strict/chull.shp")
#
### prune aquatic polys - W##
Aqua <- Class1[Class1$SemiAqLenient == "W", ]
Aqua$Species # 32
AquaPoly <- Polygons[match(Aqua$Species, Polygons$binomial), ]
AquaPolyAll <- aggregate(AquaPoly, dissolve=T)
# below is making a file for the ArbPolygons but cannot use in making the points because of class ID
AquaPolyAll.SP <- SpatialPolygonsDataFrame(AquaPolyAll, data=data.frame(binomial=1), match.ID = F)
writeOGR(AquaPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/AquaPoly_7L", layer= "chull", driver = "ESRI Shapefile")
#AquaPolyAll <-readOGR("./Analysis_Scripts/Chapter3/Polygons/AquaPoly_strict/chull.shp")
#
### prune cave polys - C##
Cave <- Class1[Class1$SemiAqLenient == "C", ]
Cave$Species # 11
CavePoly <- Polygons[match(Cave$Species, Polygons$binomial), ]
CavePolyAll <- aggregate(CavePoly, dissolve=T)
# below is making a file for the ArbPolygons but cannot use in making the points because of class ID
CavePolyAll.SP <- SpatialPolygonsDataFrame(CavePolyAll, data=data.frame(binomial=1), match.ID = F)
writeOGR(CavePolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/CavePoly_7L", layer= "chull", driver = "ESRI Shapefile")
#CavePolyAll <- readOGR("./Analysis_Scripts/Chapter3/Polygons/CavePoly_strict/chull.shp")
#
### prune saxicolous polys - S##
Sax <- Class1[Class1$SemiAqLenient == "S", ]
Sax$Species # 7
SaxPoly <- Polygons[match(Sax$Species, Polygons$binomial), ]
SaxPolyAll <- aggregate(SaxPoly, dissolve=T)
# below is making a file for the ArbPolygons but cannot use in making the points because of class ID
SaxPolyAll.SP <- SpatialPolygonsDataFrame(SaxPolyAll, data=data.frame(binomial=1), match.ID = F)
writeOGR(SaxPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/SaxPoly_7L", layer= "chull", driver = "ESRI Shapefile")
#SaxPolyAll <- readOGR("./Analysis_Scripts/Chapter3/Polygons/SaxPoly_strict/chull.shp")
#
### prune fossorial polys - F##
Foss <- Class1[Class1$SemiAqLenient == "F", ]
Foss$Species  # 3
FossPoly <- Polygons[match(Foss$Species, Polygons$binomial), ]
FossPolyAll <- aggregate(FossPoly, dissolve=T)
# below is making a file for the ArbPolygons but cannot use in making the points because of class ID
FossPolyAll.SP <- SpatialPolygonsDataFrame(FossPolyAll, data=data.frame(binomial=1), match.ID = F)
writeOGR(FossPolyAll.SP, "./Analysis_Scripts/Chapter3/Polygons/FossPoly_7L", layer= "chull", driver = "ESRI Shapefile")
#FossPolyAll <- readOGR("./Analysis_Scripts/Chapter3/Polygons/FossPoly_strict/chull.shp")
#
##############
# note that AllPolygon files have to be in SpatialPolygons format and not SpatialPolygonsDataFrame format!!!!
...
coords <- ekek@coords
maybe <- data.frame(ekek)
ArbPoints7L <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                     proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(ArbPoints7L, "./Analysis_Scripts/Chapter3/Points/Arb_Points_7L", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
#
# #MH with small res --# FOR MICROHABITAT - TERR - 3664
gridMA <- raster(extent(TerrPolyAll))
...
maybe <- data.frame(ekek)
maybe
AquaPoints7L <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                      proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(AquaPoints7L, "./Analysis_Scripts/Chapter3/Points/Aqua_Points_7L", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
...
coords <- ekek@coords
maybe <- data.frame(ekek)
CavePoints7L <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                      proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(CavePoints7L, "./Analysis_Scripts/Chapter3/Points/Cave_Points_7L", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
...
maybe <- data.frame(ekek)
maybe
SaxPoints7L <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                     proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(SaxPoints7L, "./Analysis_Scripts/Chapter3/Points/Sax_Points_7L", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
#
#MH with small res -- # FOR MICROHABITAT - FOSS 
gridMA <- raster(extent(FossPolyAll))
...
maybe <- data.frame(ekek)
maybe
FossPoints7L <- SpatialPointsDataFrame(coords = coords, data = maybe, coords.nrs = numeric(0),
                                      proj4string = CRS(as.character('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')))
writeOGR(FossPoints7L, "./Analysis_Scripts/Chapter3/Points/Foss_Points_7L", layer= "chull", driver = "ESRI Shapefile",
         overwrite=T)
